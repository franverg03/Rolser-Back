    ...
    <section class="mt-10">
        ...
        <div class="flex items-center justify-between p-4">
            <div class="contenedorTablas overflow-hidden relative">
                ...
                <input wire:model.live.debounce.100ms="search" type="search"
                    class="bg-white bordeRolser text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full tipografia-placeholder-buscar-administrativo pl-10 pr-10 p-2"
                    placeholder="Buscar pedido..." id="searchInput">
                {{-- Icono de "X" para limpiar el input --}}
                @if($search)
                    <div class="absolute inset-y-0 right-3 flex items-center cursor-pointer" id="limpiarInputBuscar"
        ...
        <div class="contenedorTablas overflow-hidden rounded-t-lg rounded-b-lg">
            <table class="w-full text-sm text-left bg-white border-collapse bordeRolser">
                <thead class="color-cabecera-tabla-admin text-xs uppercase  text-white font-bold">
                    <tr>
                        <th class="px-4 py-3 border-b bordeRolser tipografia-cabecera-tabla-administrativo">Número de pedido</th>
                        <th class="px-4 py-3 border-b bordeRolser tipografia-cabecera-tabla-administrativo">Cliente</th>
                        <th class="px-4 py-3 border-b bordeRolser tipografia-cabecera-tabla-administrativo">Razón social</th>
                        <th class="px-4 py-3 border-b bordeRolser tipografia-cabecera-tabla-administrativo">Estado</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($pedidosPaginados as $pedido)
                        <tr class="border-b bordeRolser">
                            <td class="px-4 py-3 tipografia-contenido-tabla-administrativo">{{ $pedido->id_pedido }}</td>
                            <td class="px-4 py-3 tipografia-contenido-tabla-administrativo">
                                {{ $pedido->cliente?->cliente_nombre_representante ?? 'Sin información' }}
                                {{ $pedido->cliente?->cliente_apellidos_representante ?? '' }}
                            </td>
                            <td class="px-4 py-3 tipografia-contenido-tabla-administrativo">
                                {{ $pedido->cliente?->cliente_empresa ?? 'Sin información' }}
                            </td>
                            <td class="px-4 py-3 tipografia-contenido-tabla-administrativo">{{ $pedido->pedido_estado }}</td>
                        </tr>
                    @empty
                        <tr>
