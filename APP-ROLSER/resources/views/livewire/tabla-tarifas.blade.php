    ...
    <section class="mt-10">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left bg-white border-collapse bordeRolser">
                    ...
                    <tbody>
                        @forelse ($tarifasPaginados as $tarifa)
                            <tr class="border-b bordeRolser">
                                <td class="px-4 py-3">{{ $tarifa->descripcion_tarifa }}</td>
                                <td class="px-4 py-3">{{ $tarifa->cliente->cliente_nombre_representante }} {{ $descuento->cliente->cliente_apellidos_representante }}</td>
